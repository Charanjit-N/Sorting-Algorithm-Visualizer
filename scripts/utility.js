class Utility{
    constructor(time, list =[]){
        this.time = parseInt(400/time);
        this.list = list;
    }

    mark = async (index) => {
        this.list[index].setAttribute("class", "bar current");
    }

    markMin = async (index) => {
        this.list[index].setAttribute("class", "bar min");
    }

    unmark = async (index) => {
        this.list[index].setAttribute("class", "bar");
    }


    pause = async() => {
        return new Promise(resolve => {
            setTimeout(() => {
                resolve();
            }, this.time);
        });
    }


    compare = async (index1, index2) => {
        await this.pause();
        let val1 = Number(this.list[index1].getAttribute("value"));
        let val2 = Number(this.list[index2].getAttribute("value"));
        if(val1 > val2) {
            return true;
        }
        return false;
    }

    swap = async (index1, index2) => {
        await this.pause();
        let val1 = this.list[index1].getAttribute("value");
        let val2 = this.list[index2].getAttribute("value");
        this.list[index1].setAttribute("value", val2);
        this.list[index1].style.height = `${4.0*val2}px`;
        this.list[index2].setAttribute("value", val1);
        this.list[index2].style.height = `${4.0*val1}px`;
    }

};